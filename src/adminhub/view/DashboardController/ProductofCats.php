...
@build('extra_scope_function_invokation')
    ...
    getProductofCats('{! route('api/user/product/of/cats/'.$cat_id) !}');
    CurrentCat = [ { cat: '{! $cat_name !}', id: {! $cat_id !}  } ];
    states.cats = CurrentCat[0];
    states.client_decide_price = 'false';
    states.eclient_decide_price = 'false';
    states.catProductLink = '{! route('api/user/product/of/cats/'.$cat_id) !}';
...
<div class="d-block bg-white">
    ...
    <div class="page-main">
      ...
      <div class="page-header">
        <h1 class="page-title w-75">
            {! data('Cat_Name') !} Product list(s)
            @if( data('Cat_disabled') )
                <a href="#" class="badge badge-sm badge-danger">Disabled</a>
            @endif
        </h1>
        ...
        <div class="page-header-actions">
            @if( data('Cat_disabled') )
                <a href="{! route('enable-cat/') !}{! data('Cat_Id') !}" class="btn btn-sm btn-success ml-auto">Enable</a>
            @else
                <a href="{! route('disable-cat/') !}{! data('Cat_Id') !}" class="btn btn-sm btn-danger ml-auto">Disable</a>
            @endif
          <!-- <form>
        ...
        <table ng-if="states.productList.length > 0" class="table is-indent" 
          ...
          <tbody>
            <tr ng-repeat="(key,pro) in states.productList">
              ...
              <td class="cell-300">{{pro.pname}}</td>
              <td>
                  <span ng-if="pro.is_user_define_price_unit != '1' ">{{pro.pcurrency}} {{pro.pcost | number:2}}</span>
                  <span ng-if="pro.is_user_define_price_unit == '1' ">USER_DEFINED</span>
              </td>
              ...
              <td>
                  ...
                  <a class="btn-outline-success btn-small ml-4 icon btn" ng-if="pro.is_disable == 1" href="{! route('enable-pro/') !}{{ pro.id }}"><i class="fa fa-check"></i> Enable</a>
                  <a class="btn-outline-danger btn-small ml-4 icon btn" ng-if="pro.is_disable != 1" href="{! route('disable-pro/') !}{{ pro.id }}"><i class="fa fa-times"></i> Disable</a>
              </td>
    ...
    <div class="modal-dialog modal-simple">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <p class="text-center" ng-bind-html="states.progress.productAddingformProgressNotif"></p>
          ...
          <form id="AddProductFormEx">
            ...
            <div class="form-group">
              <select class="form-control" name="pcat" id="" ng-model="states.cats" ng-options="cats.cat for (key,cats) in CurrentCat"></select>
            </div>
            ...
            <div class="form-group">
                  ...
                  <div>
                      <div class="radio-custom radio-default radio-inline">
                          <input type="radio" id="inputBasicMale" value=true ng-model="states.client_decide_price" name="client_decide_price" />
                          <label for="inputBasicMale">Yes</label>
                      </div>
                      <div class="radio-custom radio-default radio-inline">
                          <input type="radio" id="inputBasicFemale" value=false ng-model="states.client_decide_price" name="client_decide_price" ng-checked="true" />
            ...
            <div class="form-group">
              <input ng-if="states.client_decide_price == 'false' " type="number" class="form-control" name="pcost" value="0" min="0" step="0.1" required placeholder="Cost" />
            </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form id="editProductFormExt" ng-if="states.productId > 0">
            <div class="form-group">
              <input type="text" class="form-control" ng-model="states.productList[states.productIndex].pname" name="name" required placeholder="Name" />
            </div>
        ...
        <div class="modal-footer">
          <button class="btn btn-primary" ng-click="saveEditedProduct($event); editProductFormExt.$rollbackViewValue(); " data-url="{! route('api/user/product/edit/') !}{{states.productId}}" type="button">Save</button>
          <a class="btn btn-sm btn-white" data-dismiss="modal" href="javascript:void(0)">Cancel</a>
        </div>
      </div>
    </div>
